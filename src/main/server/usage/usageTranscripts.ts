 ...
 */
import type { UsageProviderKind, UsageTokenTotals } from '@shared/usage'
 ...
 * output; only a minority of lines carry usage, so this skips roughly half the
 * lines outright.
 */
 ...
 * message. Summing them overcounts (~2.4x on a real workload), so the caller
 * must drop repeats by `dedupeKey` and keep the first.
 */
