/**
 * Display formatting for the usage page, plus window construction.
 * Ported from T3 Code's usageFormat module.
 */
import type { UsageResolution, UsageSummaryInput } from './usage'
...
export function formatHourShort(hourStart: string, timeZone?: string): string {
  ...
  const hourFormat = new Intl.DateTimeFormat('en-US', { ...options, hour: 'numeric' })
...
export function formatRelativeHourShort(hourStart: string, relativeTo: string, timeZone?: string): string {
  const instant = new Date(hourStart)
 ...
 */
export function makeWindow(days: number, now = new Date(), resolution: UsageResolution = 'day'): UsageSummaryInput {
