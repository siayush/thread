...
function Tab({ threadId, surface, active }: { threadId: string; surface: RightPanelSurface; active: boolean }): JSX.Element {
  ...
  return (
    ...
    >
      {/* the icon swaps to a close button on hover, the way t3 code's tabs do */}
...
/** The "+" dropdown listing the surfaces that can be opened, styled like t3 code's. */
 ...
 * like t3 code's: D opens the diff, F the file browser.
 */
 ...
 * conversation centre, diff/files right. Replicates t3 code's right panel —
 * a per-thread tab strip over a surface switch — with the diff and files
 * surfaces.
 */
  ...
  return (
    <>
      ...
      {/* fixed full-width panel slides off-canvas when closed, so its content
          ...
          covering the chat the way t3 code collapses its chat column to w-0 */}
      <aside
        className={cn(
      ...
      >
        {/* no divider under the strip — t3 code's tab row blends into the panel */}
        <div className="drag-region flex h-13 shrink-0 items-center gap-1 px-2">
          ...
          {/* the "+" rides with the tabs, t3-code style; the gap before the window controls is the drag region */}
