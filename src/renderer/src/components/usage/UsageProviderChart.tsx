...
import type { DailyTotals, HourlyTotals, UsageProviderKind } from '@shared/usage'
import { formatDayShort, formatHourShort, formatRelativeHourShort, formatTokens, formatUsd } from '@shared/usageFormat'
...
interface PeriodColumn {
  bands: { provider: UsageProviderKind; value: number }[]
  total: number
}
...
function buildPeriodColumns(
  ...
  return periods.map((period) => {
    ...
    const bands = PROVIDER_ORDER.map((provider) => ({ provider, value: valueFor(entry, provider, metric) }))
 ...
 * peak. Rounding the maximum *up* is the point: stopping at the last step below
...
/**
 * Series are layered from a shared zero baseline, not stacked: a stacked chart
 * puts whichever provider is drawn last permanently above the other, which
 * reads as "that one is bigger" even on days where it is not.
 */
export function UsageProviderChart({
  ...
  resolution,
  timeZone
}: UsageProviderChartProps): JSX.Element {
  ...
  const { paths, ticks, stepX, toY, series } = useMemo(() => {
    if (periods.length === 0) {
      return {
        paths: [] as { provider: UsageProviderKind; area: string; line: string }[],
        ticks: [0] as number[],
        stepX: 0,
        toY: () => VIEW_HEIGHT,
        series: [] as PeriodColumn[]
      }
    }
    ...
    // Reserve a sliver above the top gridline so the series stroke, drawn at
    // constant screen width, is not shaved off at a peak.
    const toYFn = (value: number): number =>
    ...
    const built = PROVIDER_ORDER.map((provider, providerIndex) => {
      const curve = smoothCurve(
        columns.map((column, periodIndex) => ({
          x: periodIndex * step,
          y: toYFn(column.bands[providerIndex]?.value ?? 0)
        }))
      )
    ...
    // it; the strokes are drawn in a second pass regardless.
    const ordered = [...built].sort((a, b) => b.total - a.total)

    return { paths: ordered, ticks: tickValues, stepX: step, toY: toYFn, series: columns }
  ...
  const formatTooltipPeriod = (period: string): string =>
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex gap-2">
        ...
        <div ref={plotRef} className="relative h-56 flex-1" onMouseMove={handleMove} onMouseLeave={() => setHoverIndex(null)}>
          <svg
          ...
          >
            ...
            {/* Fills first, then every stroke, so no series covers another's line.
                Keyed by the period axis so switching windows replays the
                left-to-right reveal; hovering or toggling units does not. */}
            <g
              key={`${resolution}:${periods.length}:${periods[0] ?? ''}:${periods[periods.length - 1] ?? ''}`}
          ...
          {hoveredPeriod === undefined ? null : (
            <div
              className="pointer-events-none absolute top-0 z-10 min-w-36 rounded-md border border-border bg-background/95 px-2 py-1.5 text-xs"
              style={{
                left: `${hoverLeft}%`,
            ...
            >
                ...
                return (
                  <div key={provider} className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-foreground tabular-nums">
                      {format(hoveredColumn?.bands.find((band) => band.provider === provider)?.value ?? 0)}
                    </span>
                  </div>
                )
              })}
              <div className="mt-1 flex items-center justify-between gap-3 border-t border-border pt-1">
                ...
                <span className="text-foreground tabular-nums">{format(hoveredColumn?.total ?? 0)}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex justify-between pl-16 text-[10px] text-muted-foreground uppercase">
        ...
        <span>{periods[periods.length - 1] === undefined ? '' : formatPeriod(periods[periods.length - 1] ?? '')}</span>
      </div>
    </div>
  )
}

export function UsageChartLegend(): JSX.Element {
  return (
